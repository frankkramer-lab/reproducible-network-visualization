...
TODO: rename all node attributes:
GSMxxxxxx_Score
GSMxxxxxx_GE
GSMxxxxxx_GE_Level
GSMxxxxxx_GE_StdDev
...
TODO: rename network attributes

TODO: MTB results: drugs as nodes in network!!!
...
Helper function to print attributes independently from the patient ids
...
printAttributes(ppi_network_rcx$networkAttributes)
```
...
printAttributes(ppi_integrated_network_rcx$networkAttributes)
```
#TODO: remove
```{r}
tmp_ppi_integrated_network_rcx = ppi_integrated_network_rcx
ppi_integrated_network_rcx = updateNetworkAttributes(
  ppi_integrated_network_rcx, 
...
printAttributes(ppi_integrated_network_rcx$networkAttributes)
ppi_integrated_network_rcx$networkAttributes = ppi_integrated_network_rcx$networkAttributes[ppi_integrated_network_rcx$networkAttributes$name!="OccurrenceBySubtype",]
...
```{r getThreshold, eval=FALSE, include=FALSE}
topNr=140
...
thresholds = data.frame(
  patients = names(thresholds),
  subtype = patients$subtype,
  thresholds = thresholds
)
# boxplot(thresholds, horizontal = T)
boxplot(thresholds$thresholds)
boxplot(thresholds~subtype, data = thresholds)
```
...
combined_subnetwork_rcx = ppi_integrated_network_rcx

all_rel_genes = unique(unlist(relevant_genes))

tmp_nodes = combined_subnetwork_rcx$nodes
tmp_edges = combined_subnetwork_rcx$edges
...
combined_subnetwork_rcx$nodes = tmp_nodes
combined_subnetwork_rcx$edges = tmp_edges

## remove all attributes not included in the nodes
combined_subnetwork_rcx$nodeAttributes = combined_subnetwork_rcx$nodeAttributes[
  combined_subnetwork_rcx$nodeAttributes$propertyOf %in% combined_subnetwork_rcx$nodes$id,
]

for(patientId in names(relevant_genes)){
  ## get ids of relevant genes for patients
  rel_gene_ids = combined_subnetwork_rcx$nodes$id[
    combined_subnetwork_rcx$nodes$name %in% relevant_genes[[patientId]]
  ]
  
  ## select node attributes relevant for patient
  sel_node_attr = startsWith(combined_subnetwork_rcx$nodeAttributes$name, patientId)
  keep_node_attr = combined_subnetwork_rcx$nodeAttributes$propertyOf %in% rel_gene_ids
  remove_node_attr = sel_node_attr & !keep_node_attr
  
  combined_subnetwork_rcx$nodeAttributes = combined_subnetwork_rcx$nodeAttributes[!remove_node_attr,]
}

combined_subnetwork_rcx = updateMetaData(combined_subnetwork_rcx)

combined_subnetwork_rcx$metaData
...
getOccurrencesBySubtype = function(network){
  ...
  occurrences = dlply(occurrences, 1, function(x){list(x[-1])})
...
tmp_combined_subnetwork_rcx = combined_subnetwork_rcx
combined_subnetwork_rcx = updateNetworkAttributes(
  combined_subnetwork_rcx, 
  createNetworkAttributes(
    name = c(
...
printAttributes(combined_subnetwork_rcx$networkAttributes)
```

```{r writeBigSubnetwork, eval=FALSE, include=FALSE}
## Save the data
writeCX(combined_subnetwork_rcx, "tmp/combined_subnetwork_rcx.cx")
saveRDS(combined_subnetwork_rcx, "tmp/combined_subnetwork_rcx.RDS")
```
...
subnetworks = lapply(names(relevant_genes), function(patientId){
  ...
  subnet = combined_subnetwork_rcx
...
#' Build a concentric layout for a RCX network
#'
#' @param rcx_network RCX network
#' @param dX horizontal distance between nodes
#' @param dY vertical distance between nodes
#'
#' @return cartesianLayoutAspect
...
cartesianLayout = buildLayout(combined_subnetwork_rcx)
tmp = updateCartesianLayout(
  combined_subnetwork_rcx,
  cartesianLayout
)
writeCX(tmp, "tmp/combined_with_cartesian_layout.cx")
visualize(tmp)
```
