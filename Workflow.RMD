...
for(patientId in patients$geo_accession){
  ...
  if(nrow(mtb_results[[patientId]]) != 0){
    ...
    for(gene in genes){
      ...
      tmp_gene_id = big_subnetwork$nodes$id[big_subnetwork$nodes$name == gene]
...
big_subnetwork = updateNetworkAttributes(
  big_subnetwork,
...
## Save the data
writeCX(big_subnetwork, "tmp/big_subnetwork.cx")
saveRDS(big_subnetwork, "tmp/big_subnetwork.RDS")
```


```{r bigSubnetworkNDExLoad, eval=FALSE}
big_subnetwork = ndex_get_network(ndex_con, "a420aaee-4be9-11ec-b3be-0ac135e8bacf")
```
